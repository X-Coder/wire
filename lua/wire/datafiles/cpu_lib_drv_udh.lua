local path = "CPUChip/lib/drivers/drv_udh.txt"
...
void udhQueryDevices() {
  ...
  for (i = 0; i < 8; i++) {
    udhBusOffset[i*2+0] = MAX_UDH_ADDRESS_RANGE*i;
    udhBusOffset[i*2+1] = MAX_UDH_ADDRESS_RANGE*i+(MAX_UDH_ADDRESS_RANGE-1);
  }
